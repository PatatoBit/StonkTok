...
serve(async ()=>{
  ...
  try {
    const { data: videos, error: fetchErr } = await supabase.from('videos').select('id, video_url, platform');
    if (fetchErr) {
    ...
    for (const video of videos){
      try {
        ...
        const { data: scrapeData, error: scrapeErr } = await supabase.functions.invoke(platform === 'tiktok' ? 'tiktok-apify' : 'instagram-apify', {
          body: {
            videoUrl: video_url
          }
        });
